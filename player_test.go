...
func TestPlayer_NewPlayerHasIdentity(t *testing.T) {
	...
	assert.Equal(t, player.Name(), pattay.SouthPlayer)
}
...
func TestPlayer_ThrowCardAndReceiveCardOnTable(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(p1, p2 pattay.Player, i int) {
			p1.ThrowCard(i)
			p2.ThrowCard(i)
		}(p1, p2, i)
	}
...
func TestPlayer_HasAnySpade(t *testing.T) {
	...
	p := pattay.NewPlayer(pattay.WestPlayer)
...
func TestPlayer_ErrorOnPlayerReceivesFourteenCards(t *testing.T) {
	player := pattay.NewPlayer(pattay.WestPlayer)
	deck := pattay.NewDeck()
